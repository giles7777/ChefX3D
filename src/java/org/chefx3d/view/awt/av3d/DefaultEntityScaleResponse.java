...
/**
 * Responds to scale events.
 *
 * @author Jonathon Hubbard
 * @version $Revision: 1.6 $
 *
 */
public class DefaultEntityScaleResponse implements
	 ...
	 * @param editorGrid
     */
    public DefaultEntityScaleResponse(
        WorldModel model,
    ...
    public void doEventResponse(
        ...
        for(int i = 0; i < entities.length; i++) {
            ...
            if ((vertical_delta != 0) || (horizontal_delta != 0)) {
                ...
                ArrayList<PositionableData> startPositions = new ArrayList<PositionableData>();
                if (entityDataList != null && entityDataList.size() - 1 >= i) {
                    startChildren = actionData.entityList.get(i).startChildren;
                    startPositions = actionData.entityList.get(i).startPositions;
                }
                ...
                ScaleEntityCommand scaleCmd =
                    new ScaleEntityCommand(
                        ...
                        newEntityScale,
                        originalEntityScale,
                        startChildren, startPositions);

                
				boolean align = editorGrid.alignPositionToGrid(newEntityPosition);
				if (align) {
					
					originalEntityPosition[0] = newEntityPosition[0];
					originalEntityPosition[1] = newEntityPosition[1];
					originalEntityPosition[2] = newEntityPosition[2];
