...
/**
 * Responds to Entity move events. This is only for transient move
 * events. The appropriate command is issued for the event.
 *
 * @author Ben Yarger
 * @version $Revision: 1.22 $
 */
public class EntityMoveTransientResponse implements TrackerEventResponse {
	 ...
	 * @param editorGrid
     */
    public EntityMoveTransientResponse(
        WorldModel model,
        ...
        for (int i = 0; i < entities.length; i++) {
			...
			if (entity != null) {
				...
				if (entity instanceof VertexEntity) {
					...
					cmd = new MoveSegmentTransientCommand(
						model,
						model.issueTransactionID(),
						segment,
						newEntityPosition,
