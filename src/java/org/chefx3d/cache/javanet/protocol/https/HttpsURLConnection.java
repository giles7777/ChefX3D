 ...
 *
 ****************************************************************************/
package org.chefx3d.cache.javanet.protocol.https;
...
public class HttpsURLConnection extends javax.net.ssl.HttpsURLConnection {
    ...
    private sun.net.www.protocol.https.HttpsURLConnectionImpl httpConnection = null;
    ...
    private void setupConnection() {
        ...
        try {
            ...
            if (!connected) {
                ...
                if ("GET".equals(httpConnection.getRequestMethod()) && protocolUtils.isCachableURL(url)) {
                    if (protocolUtils.doesAssetExist(url)) {
                        if (protocolUtils.checkFreshness()) {
                            httpConnection.setIfModifiedSince(protocolUtils.getLastModified(url));
                            log.debugReport("Checking freshness", null);
                        } else {
                            // Not checking freshness, simply return cached values.
                            log.debugReport("No freshness check, loading from cache", null);
                            initializeFromCache();
                            return;
                        }
                    }
                    ...
                    try {
                        ...
                        log.debugReport("Got response " + httpConnection.getResponseCode(), null);
                        if (httpConnection.getResponseCode() == HTTP_OK) {
                            log.debugReport("Downloading content for caching!", null);
                            iStream = httpConnection.getInputStream();
                            processWrappedConnectionHeaders();
                            protocolUtils.storeHeadersInCache(url, keys, headers);
                            protocolUtils.storeAsset(url, iStream);
                            // Now that it is cached, use the cache to look up the values.
                            initializeFromCache();
                            return;
                        } else if (httpConnection.getResponseCode() != HTTP_NOT_MODIFIED) {
                            // Still try and set up headers, get inputstream?
                            processWrappedConnectionHeaders();
                            log.warningReport("URL " + url + " is not in cache, but got response code " + httpConnection.getResponseCode() + " when trying to download!", null);
                            return;
                        }
                    } catch (Exception ex) {
                        log.warningReport("Caught exception while trying to download from host, reusing potentially stale value from cache for " + url, null);
                    }
                    initializeFromCache();
                } else {
                    // Do the usual...
    ...
    private void initializeFromWrappedConnection() {
        ...
        try {
            ...
            log.errorReport("Error trying to use wrapped connection!", null);
    ...
    protected HttpsURLConnection(sun.net.www.protocol.https.HttpsURLConnectionImpl httpConnection, URL url) {
        super(url);
    ...
    @Override
    public String getCipherSuite() throws IllegalStateException {
        return httpConnection.getCipherSuite();       
    }
    
    @Override
    public Certificate[] getLocalCertificates() {
        return httpConnection.getLocalCertificates();       
    }
 
    @Override
    public Certificate[] getServerCertificates() throws SSLPeerUnverifiedException {
        return httpConnection.getServerCertificates();          
    }
