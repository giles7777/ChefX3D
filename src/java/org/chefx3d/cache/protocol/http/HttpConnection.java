...
//      current_connection.setAllowUserInteraction(allowUserInteraction);
            ...
            try {
                if (GET_METHOD.equals(method)) {
                    ...
                    if (protocolUtils.checkFreshness() &&
                          protocolUtils.doesAssetExist(getURI())) {

                        NVPair nVPair = new NVPair("If-Modified-Since",
                              protocolUtils.getLastModifiedString(getURI()));

                        response = current_connection.Get(resource,
                              new NVPair[0], new NVPair[]{nVPair});
    ...
    private void parseHeaders() {
        ...
        if (protocolUtils.doesAssetExist(getURI()) && GET_METHOD.equals(method) && ! protocolUtils.checkFreshness()) {
