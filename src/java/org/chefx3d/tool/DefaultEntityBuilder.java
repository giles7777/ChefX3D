...
public class DefaultEntityBuilder extends EntityBuilder {
    ...
    public Entity createEntity(
        ...
        if (tool instanceof SegmentableTool) {
            ...
            newEntity =
                createSegmentableEntity(entityID, (SegmentableTool)tool);

        } else if (tool instanceof BuildingTool) {

            newEntity =
                createBuildingEntity(model, entityID, position, rotation, (BuildingTool)tool);

        } else if (tool instanceof EntityGroupTool) {

            newEntity =
                createEntityGroupEntity(model, position, rotation, (EntityGroupTool)tool);

        } else if (tool instanceof VertexTool) {
            
            newEntity = createVertexEntity(entityID, position, rotation, (SimpleTool)tool);
            
        } else if (tool instanceof SegmentTool) {
            
            newEntity = createSegmentEntity(entityID, position, rotation, (SimpleTool)tool);
            
        } else if (tool instanceof PasteTool) {
            
            newEntity = createPasteEntity(position, rotation, (PasteTool)tool);
            
        } else {

            newEntity = createDefaultEntity(entityID, position, rotation, (SimpleTool)tool);
     ...
     * @return
     */
    protected Entity createPasteEntity(
            double[] position,
            float[] rotation, 
            PasteTool tool) {
        
        Entity entity = tool.getPasteEntity();
                
        // set the position if necessary
        if (entity instanceof PositionableEntity) {
            ((PositionableEntity)entity).setPosition(position, false);
            ((PositionableEntity)entity).setRotation(rotation, false);
        }
